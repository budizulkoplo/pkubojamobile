                    ...
                    <div class="card-body">
                        <div class="row text-center">
                            ...
                            <div class="col-6 col-md-3 mb-3">
                                <div class="card bg-light">
                                    <div class="card-body py-2">
                                        <h6 class="mb-1">Infaq Terlambat</h6>
                                        <h4 class="text-danger mb-0">Rp {{ number_format($infaqTerlambatBerjalan ?? 0, 0, ',', '.') }}</h4>
                                        <small class="text-muted">
                                            {{ \App\Libraries\PayrollLateInfaq::formatMinutes($lateMinutesTotal ?? 0) }}
                                            @if(($infaqTerlambatRate ?? 0) > 0)
                                                · {{ number_format(($infaqTerlambatRate ?? 0) * 100, 0) }}%
                                            @endif
                                        </small>
                                    </div>
                                </div>
            ...
            <div class="col-12">
                ...
                <div class="alert alert-info mt-3 mb-0">
                    <small>
                        ...
                        <br>
                        Terlambat mengikuti batas masuk yang berlaku di kalender, termasuk aturan 30 menit sebelum jam kerja setelah tanggal aktif.
                        <br>
                                ...
                                <tbody>
                                    @foreach($dataKalender as $tgl => $item)
                                        @if(!empty($item['jam_masuk']) || !empty($item['jam_pulang']) || !empty($item['status_khusus']))
                                            @php
                                                $lateSeconds = empty($item['status_khusus']) ? (int) ($item['late_seconds'] ?? 0) : 0;
                                                $lateText = $lateSeconds > 0 ? \App\Libraries\PayrollLateInfaq::formatMinutes((int) floor($lateSeconds / 60)) : '-';
                                                $cutoff = $item['late_cutoff_time'] ?? '';
                                            @endphp
                                            <tr>
                                                <td>{{ \Carbon\Carbon::parse($tgl)->translatedFormat('d M Y') }}</td>
                                                <td>{!! $item['jam_masuk'] ?? '-' !!}</td>
                                                <td>{{ $item['jam_pulang'] ?? '-' }}</td>
                                                <td class="text-center {{ $lateSeconds > 0 ? 'text-danger fw-bold' : '' }}">
                                                    {{ $lateText }}
                                                    @if($lateSeconds > 0 && !empty($cutoff))
                                                        <br><small class="text-muted">Batas {{ \Carbon\Carbon::parse($cutoff)->format('H:i') }}</small>
                                                    @endif
                                                </td>
                                                <td align="center">
