...
<div class="row" style="margin-top:70px">
    <div class="col">
        ...
        @if (!empty($dataKalender))
            @if(($lateInfaq['active'] ?? false) || ($lateInfaq['amount'] ?? 0) > 0)
                <div class="alert alert-warning d-flex justify-content-between align-items-center">
                    <span>Infaq terlambat berjalan</span>
                    <strong>Rp {{ number_format($lateInfaq['amount'] ?? 0, 0, ',', '.') }}</strong>
                </div>
            @endif
                            ...
                            <tr>
                                @foreach ($mingguKe as $tgl)
                                    @php
                                        ...
                                        $lateSeconds = empty($data['status_khusus']) ? (int) ($data['late_seconds'] ?? 0) : 0;
                                        ...
                                        <div class="cell-content">
                                            @if ($showAttendance)
                                                ...
                                                @if ($lateSeconds > 0 && empty($data['status_khusus']))
                                                    ...
                                                    @if(!empty($data['late_cutoff_time']))
                                                        <div class="terlambat">Batas: {{ $formatWaktu($data['late_cutoff_time']) }}</div>
                                                    @endif
                                                @endif
