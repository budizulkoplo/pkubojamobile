...
<div class="appHeader bg-primary text-light">
    <div class="left">
        <a href="{{ route('hris.jadwal_operasi') }}" class="headerButton goBack">
            <ion-icon name="chevron-back-outline"></ion-icon>
        </a>
    </div>
    <div class="pageTitle">Absen Operasi</div>
    <div class="right"></div>
</div>
    ...
    <div class="row">
        <div class="col">
            <div class="text-center mb-2">
                <h5>{{ $operasi->nama_pasien }}</h5>
                <p class="text-muted">{{ $operasi->nama_kamar }}</p>
                <p><strong>{{ \Carbon\Carbon::parse($operasi->tanggal_rencana)->translatedFormat('d F Y H:i') }}</strong></p>
            </div>

            <div class="section-title">Ambil Foto Absen Operasi</div>
...
<div class="sticky-absen-btn">
    <button id="operasiIn" class="btn btn-primary">
        <ion-icon name="camera-outline"></ion-icon> Operasi In
    </button>
    <button id="operasiOut" class="btn btn-danger">
        <ion-icon name="exit-outline"></ion-icon> Operasi Out
    </button>
</div>
...
// 🗺️ Inisialisasi Lokasi & Area Absensi
        ...
        fetch("{{ route('hris.absensilock.get') }}")
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    lockMarker = L.marker([latLock, lngLock]).addTo(map).bindPopup("Titik Area Operasi");
                    ...
                    Swal.fire('Error', 'Gagal memuat titik area operasi.', 'error');
                }
            })
            .catch(() => Swal.fire('Error', 'Gagal mengambil data area operasi.', 'error'));
    }, () => {
...
function prosesAbsen(mode) {
    ...
    if (!window.absensiLockLat || !window.absensiLockLng || !window.absensiLockRadius) {
        Swal.fire('Area Belum Siap', 'Titik area belum dimuat. Coba refresh halaman.', 'warning');
        return;
    }
    ...
    if (jarak > radius) {
        Swal.fire(
            'Di Luar Area!',
            'Anda berada ' + Math.round(jarak) + ' m dari titik area operasi.',
            'warning'
        );
        return;
    }
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('hris.operasi_absen_store') }}",
        data: {
            _token: "{{ csrf_token() }}",
            id: "{{ $operasi->id }}",
            no_rm: "{{ $operasi->no_rm }}",
        ...
        success: function(res) {
            ...
            if (res.status === 'success') {
                ...
                setTimeout(() => {
                    window.location.href = "{{ route('hris.jadwal_operasi') }}";
                }, 2500);
            } else {
...
// 🔘 Event Button
$("#operasiIn").click(() => prosesAbsen('in'));
$("#operasiOut").click(() => prosesAbsen('out'));
