...
use Carbon\Carbon;
use App\Libraries\PayrollLateInfaq;
...
class KalenderController extends Controller
{
    ...
    public function statistik(Request $request)
    {
        ...
        if ($this->selectedEmployee) {
            ...
            $viewData = array_merge($viewData, [
                ...
                'infaqTerlambatBerjalan' => $lateInfaq['amount'],
                'infaqTerlambatRate' => $lateInfaq['rate'],
                'infaqTerlambatAktif' => $lateInfaq['active'],
                'lateMinutesTotal' => $lateInfaq['late_minutes'],
                'lateRuleActiveDate' => $lateInfaq['tgl_aktif'],
            ]);
        }
    ...
    protected function prepareKalenderData(string $bulan, bool $preferNewModel = true): array
    {
        if ($preferNewModel) {
    ...
    protected function processDoubleShiftStats(array &$stats, array $data): void
{
    ...
    protected function calculateRunningLateInfaq(string $pegawaiPin, string $bulan, int $lateSeconds, int $workDays): array
    {
        $lateMinutes = (int) floor(max(0, $lateSeconds) / 60);
        $tglAktif = DB::table('keterlambatan')
            ->whereNotNull('tgl_aktif')
            ->min('tgl_aktif');

        $uangMakanNominal = (float) (DB::table('nominaldasar')->value('uangmakan') ?? 0);
        $uangMakan = $workDays * $uangMakanNominal;

        $masterGaji = DB::table('mastergaji')
            ->where('pegawai_pin', $pegawaiPin)
            ->where('verifikasi', '1')
            ->whereRaw("DATE_FORMAT(tglaktif, '%Y-%m') <= ?", [$bulan])
            ->orderByDesc('tglaktif')
            ->first();

        $isHarian = ($masterGaji->harian ?? 0) == 1;
        $isDirektur = ($masterGaji->direktur ?? 0) == 1;
        $isTraining = ! $isHarian && ($masterGaji->gajipokok ?? 0) <= 0;

        if ($isHarian || $isDirektur || $isTraining) {
            $uangMakan = 0;
        }

        return [
            'amount' => PayrollLateInfaq::calculate($lateMinutes, $uangMakan, $tglAktif, $bulan),
            'rate' => PayrollLateInfaq::getRate($lateMinutes),
            'active' => PayrollLateInfaq::isActiveForPeriod($tglAktif, $bulan),
